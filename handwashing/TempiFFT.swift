...
//        let iFloats = [Float](UnsafeBufferPointer(start: self.complexBuffer.imagp, count: self.halfSize))
    ...
    func max_freq_idx() -> Int {
        var max_amp: Float = 0.0
        var max_amp_idx = 0
        
        for i in 0...(size/2 - 1) {
            if magnitudes[i] > max_amp {
    ...
    func max_val() -> Float {
        //return magnitudes[max_freq_idx()]
        return magnitudes[836]
      }
      
    
    func lr_width() -> (l_width: Int, r_width: Int) {
        var i = 0
        let max_amp_idx = 836
        while max_amp_idx + i < size/2 {
            if magnitudes[max_amp_idx + i] < 0.01*magnitudes[max_amp_idx] {
                break
            }
            i = i + 1
        }
    ...
    func side_mags() -> Float {
        var total: Float = 0.0
        for i in 2...4 {
            total += magnitudes[836 + i]
            total += magnitudes[836 - i]
        }
        return total
    }
